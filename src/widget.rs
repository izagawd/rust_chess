...
pub trait Widget : Any{
    ...
    fn get_scene(&self) -> Rc<dyn Scene>{
        self.widget_data().borrow().scene.as_ref().unwrap().upgrade().unwrap()
    }
...
default impl<T: 'static> Widget for T {
    ...
    fn size(&self) -> Vector2<f32>{
        self.widget_data().borrow().size.clone()
    }
    fn set_local_position(&self, value: WidgetVector) {
        self.widget_data().borrow_mut().widget_position= value;
    }
    fn local_position(&self) -> WidgetVector{
        self.widget_data().borrow().widget_position.clone()
    }
    fn set_size(&self, value: Vector2<f32>){
        self.widget_data().borrow_mut().size= value;
    }
    fn global_position(&self) -> Vector2<f32>{
        ...
        let widget_pos = self.widget_data().borrow().widget_position.clone();
    ...
    fn get_parent(&self)->Option<Rc<dyn Widget>>{
        let kk =self.widget_data().borrow().parent.clone();
        if let Some(k) = kk {
            return k.upgrade();
        }
        return None;
    ...
    fn get_children(&self)->Ref<Vec<Rc<dyn Widget>>>{
        Ref::map(self.widget_data().borrow(),|x| &x.children)
    }
    fn set_parent(self: Rc<Self>,parent: Option<Rc<dyn Widget>>) -> Result<(),&'static str> {
        ...
        if let Some(to_become_parent) = parent{
            ...
            if to_become_parent.widget_data().borrow().children.iter().any(
                |w| Rc::ptr_eq(&as_widget, &w)) {
                return Ok(())
            }
            let mut curr_parent = to_become_parent.widget_data().borrow() .parent.clone();
            ...
            to_become_parent.widget_data().borrow_mut().children.push(self.clone());
            let prev_parent = self.widget_data().borrow().parent.clone().and_then(|x| x.upgrade());
            if let Some(prev_parent) = prev_parent {
                let index = prev_parent.widget_data().borrow().children.iter().position(|x|
                Rc::ptr_eq(&as_widget,x));
                if let Some(index) = index {
                    prev_parent.widget_data().borrow_mut().children.remove(index);
                }

            }

            self.widget_data().borrow_mut().parent = Some(Rc::downgrade(&to_become_parent));
            Ok(())
        } else{
            let curr_parent = self.widget_data().borrow().parent.clone().and_then(|x| x.upgrade());

            if let Some(true_one) = curr_parent {

                let gotten = true_one.widget_data().borrow().children.iter()
            ...
            self.widget_data().borrow_mut().parent = None;
            return Ok(());
