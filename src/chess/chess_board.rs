...
impl ChessBoard{
    ...
    pub fn new(current_scene: &dyn Scene) -> Rc<ChessBoard>{
       let mut created = ChessBoard{selected_slot: RefCell::new(None), chess_slots: Vec::new(), widget_data: WidgetData::default()};
        ...
        for i in created.chess_slots.iter().filter(|x| x.get_slot_position().y == 0){
            let created_pawn =      add_widget(current_scene,Pawn::new(Black));
           i.clone().set_piece_at_slot(Some(created_pawn));
        }

        //adding white pawns..
        for i in created.chess_slots.iter().filter(|x| x.get_slot_position().y == 7){
            let created_pawn =      add_widget(current_scene,Pawn::new(White));
            i.clone().set_piece_at_slot(Some(created_pawn));
        }
